...
- **Estado:** review (hitos 1–6 hechos)
...
- ✅ **Hito 6 · Balance fino (22/09/2026):** perfil de balance `romance` (`familyBias` + `cheatChance: 0.25`, `engine/balance.ts`): el bot puede ir por una acción romántica con alguien que no es su pareja (antes las evitaba siempre, `AutoOpts.cheatChance`, 0 por defecto → sin cambios en los demás perfiles). Métricas nuevas `pct.divorced` y `pct.caughtCheating` (flag `caught_cheating`, sumado en los 6 desenlaces de ruptura de `love.cheat_discovered`). Con 100–300 vidas simuladas: `caughtCheating` ronda 17–28 % y `divorced` 17–22 %, sin romper `married`/`children` ni ninguna otra banda. Test de banda en `balance.bands.test.ts`. La cifra la fija `cheatChance`; si Max la quiere más baja, se ajusta ese número.
...
- ✅ **Eventos y acciones viejos sincronizados:** condiciones de amistad en los eventos que suponen buena relación (`rel.friend_*`, `rel.mother_visit`, `rel.father_health`), de amor en los de pareja (que ahora usan `fx.bond`), `rel.ex_gossip` solo si no se llevan bien, y sospecha (`fx.suspect`, nuevo efecto) en `love.temptation`, `work.affair_offer`, `hookup` y `cheat`. `firstAlive` elige al amigo/hermano/hijo con mejor amistad.
- ✅ **La IA sincronizada (22/09/2026):** ver `docs/05-ia.md` § "La IA sincronizada con amistad, amor y enemistad". Eventos generados con `"person"` (target + condición de amistad, placeholder `{target}` obligatorio), responder escribiendo puede mover la amistad/amor de la persona de la situación, y `summarizeLife` suma contexto de relaciones. 41 tests (`ai/ai.test.ts`, `ai/freetext.test.ts`).
