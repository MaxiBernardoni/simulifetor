...
**Origen de los cambios del año (T18)**: `life.lastDelta` (el total, sin cambios) va acompañado de `life.lastDeltaSources?: DeltaSource[]` — el mismo total desglosado por causa. `addLog` guarda `ctx.deltas` junto al texto cuando existen (evento sin decisiones, resultado de una decisión o de una actividad/acción); al final de `ageUp`, `deltaSources` recorre lo logueado desde el principio del año y le resta cada aporte al total real; lo que sobra (impuestos, mantenimiento, el desgaste natural de las stats…) queda en una entrada "Otros cambios", así el desglose siempre suma exactamente lo que muestra el chip. En la pantalla, tocar un chip de `life.lastDelta` despliega esa lista (`DeltaChips`, `ui/components.tsx`); los chips de un resultado puntual (`prompt.deltas`) no lo necesitan, porque ya son una sola causa.
