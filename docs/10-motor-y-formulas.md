...
2. **`agePeople`**: cada persona envejece (`p.age++`). Las que **no** son del árbol mueren con `baseMortality(edad) × 0,9`; las del árbol (`nodeId`) solo se enfrían (la muerte la decide el mundo). Luego: amigos/ex con cercanía ≤ 8 se pierden; pareja con cercanía ≤ 15 corta (50 %/año; si estaban casados, divorcio con pérdida del 30 % del dinero).
