  ...
  { id: 'love.cheat_discovered', title: 'Te descubrieron', tags: ['love'], target: 'partner', weight: 1,
    ...
    choices: [
      { label: 'Negarlo todo', outcomes: [
        { weight: 3, text: '{target} te creyó a medias. Quedó una duda que no se va a ir.', effects: [fx.bond('target', -6, -8), fx.hap(-2)] },
        { weight: 7, text: '{target} tenía capturas de pantalla. Fue una escena que nadie olvida.', effects: [fx.bond('target', -25, -30), fx.hap(-6)] },
        { weight: 4, conditions: [c.single()], text: '{target} juntó sus cosas y se fue esa misma noche. Se terminó.', effects: [fx.becomes('target', 'ex'), fx.hap(-10)] },
        { weight: 4, conditions: [c.married()], text: '{target} pidió el divorcio con un abogado ya contratado. Salió caro.', effects: [fx.becomes('target', 'ex'), fx.flag('divorced'), fx.moneyPct(-0.25), fx.hap(-10)] },
      ] },
      { label: 'Pedir perdón de rodillas', outcomes: [
        { weight: 5, text: '{target} lloró, gritó y te perdonó. Con condiciones.', effects: [fx.bond('target', -10, -15), fx.hap(-3)] },
        { weight: 3, text: '{target} te dio una última oportunidad. La palabra es "última".', effects: [fx.bond('target', -18, -20), fx.hap(-4)] },
        { weight: 2, conditions: [c.single()], text: '{target} escuchó todo y aun así se fue. Se terminó.', effects: [fx.becomes('target', 'ex'), fx.hap(-9)] },
        { weight: 2, conditions: [c.married()], text: '{target} escuchó todo y aun así pidió el divorcio.', effects: [fx.becomes('target', 'ex'), fx.flag('divorced'), fx.moneyPct(-0.25), fx.hap(-9)] },
      ] },
      { label: 'Admitirlo y decir que no es para tanto', outcomes: [
        { weight: 3, text: '{target} te pegó un portazo que hizo temblar las paredes. Todavía no se fue.', effects: [fx.bond('target', -30, -35), fx.hap(-5)] },
        { weight: 7, conditions: [c.single()], text: '{target} se rió con amargura y se fue para siempre.', effects: [fx.becomes('target', 'ex'), fx.hap(-8)] },
        { weight: 7, conditions: [c.married()], text: '{target} pidió el divorcio y se quedó con la mitad de todo.', effects: [fx.becomes('target', 'ex'), fx.flag('divorced'), fx.moneyPct(-0.25), fx.hap(-8)] },
      ] },
    ] },
];
