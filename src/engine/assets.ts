...
export function canBuy(life: Life, catalogId: string, financed: boolean): string | null {
  ...
  if (financed) {
    const down = Math.round(item.price * DOWN_PAYMENT);
    if (life.money < down) return `Necesitás ${formatMoney(down)} de entrada`;
    if (loanCapacity(life) < item.price - down) return 'El banco no te aprueba el crédito';
  } else if (life.money < item.price) {
    return `Necesitás ${formatMoney(item.price)}`;
  }
  return null;
}
...
export function updateAssets(life: Life, rng: Rng): void {
  for (const a of life.assets) {
    a.value = Math.round(a.kind === 'house' ? a.value * (1 + (rng.int(-4, 9) / 100)) : a.value * 0.9);
...
export function housingCost(life: Life): number {
  ...
  return house ? 2000 + Math.round(house.value * 0.01) : 7000;
}

export function carCost(life: Life): number {
  return life.assets.filter((a) => a.kind === 'car').length * 900;
}
